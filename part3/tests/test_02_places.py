...
class TestPlaces:
    def test_create_place(self, auth_client: AuthenticatedClient, shared_data: SharedData):
        ...
        shared_data.place_payload["owner_id"] = shared_data.user_id

        check_multiple_action([
            { "owner_id": "test", "status": 403, "excepted_payload": { "error": "Unauthorized action." } },
            { "price": -1 },
            { "latitude": -180 },
            { "longitude": 2000 }
    ...
    def test_get_all_places(self, auth_client: AuthenticatedClient, shared_data: SharedData):
        ...
        check_response(
            ...
            status_code=200,
            array_template=array_without_value(shared_data.place_template(), ["price", "owner_id", "description"]),
            array_payload=[
                dict_without_keys(shared_data.place_payload, ["price", "owner_id", "description"])
            ],
            partial_match=True
        )
